function cursosController() {
  return {
    ...
    lastFormIndex: Number($('#formCursos').find('.fields-container').last().attr('data-index')),

    // Inicia o formulário
    init() {
      this.base = $forms.getFormBase($('#formCursos')).clone()
    },

    // Adicionar um novo curso
    new() {
      $forms.cloneForm('formCursos', ++this.lastFormIndex)
    },
    ...
    insertMedia(data, mediaIndex) {
      // Exibe o Alert
      $forms.showAlert(data, 'formCursos', mediaIndex)
      $forms.hideLinkField('formCursos', mediaIndex)
      $forms.disableMediaButtons('formCursos', mediaIndex)
      $forms.addMediaAttr('formCursos', mediaIndex, data);

      var ctx = this;

      // Convert a foto pra base64 e adiciona nos atributos
      $forms.convertToBase64(data.foto).then(function (r) {
        $forms.addMediaAttr('formCursos', mediaIndex, r.foto);
    ...
    removeMedia(formIndex) {
      // Apaga o alert
      $forms.hideAlert("formCursos", formIndex)
      $forms.enableMediaButtons("formCursos", formIndex)

      // Apaga a mídia
      $forms.prepareRequest('/cursos/' + formIndex + ' /certificado/', null, 'DELETE') 
      .then(function(){
        $("#formCursos .fields-container[data-index='" + mediaIndex + "']").removeAttr('data-midia')
      })
    ...
    submit() {
      ...
      dataArr.forEach(function (curso) {

        var reqType = curso.isNewData ? 'POST' : 'PUT'

        if (reqType == "POST") {
          requestPromises.push(
            $forms.prepareRequest('/cursos/', curso, reqType)
          )
        } else {
          requestPromises.push(
            $forms.prepareRequest('/cursos/' + curso.formIndex + '/', curso, reqType)
          )
        }
      })
    ...
    remove(id, newForm) {
      ...
      $forms.prepareRequest('/cursos/' + id + '/', null, 'DELETE')
    }
